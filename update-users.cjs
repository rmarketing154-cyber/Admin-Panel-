...
// Update imports
content = content.replace("import { Users as UsersIcon", "import { Users as UsersIcon, ChevronLeft");

// Change modal to full-page view
// Find where the modal starts: {/* User Details Modal Overlay */}
const modalMarker = '{/* User Details Modal Overlay */}';
const modalIdx = content.indexOf(modalMarker);
...
if (modalIdx !== -1 && returnIdx !== -1) {
  // First, extract the content of the modal so we can reuse parts, then replace the whole return.
...
`;
  ...
  // First we need to remove totalWdAmount, userSubsCount, refsCount declarations from outside
  content = content.replace(/  const totalWdAmount[\s\S]*?refsCount = [^\n]*;\n/s, '');
  
  // Then we need to remove the modal from the bottom
  const mainReturnEnd = content.indexOf('{/* User Details Modal Overlay */}');
  if(mainReturnEnd !== -1) {
    let returnBlock = content.substring(returnIdx, mainReturnEnd);
    returnBlock += `    </div>\n  );\n}`;
    
    // Combine everything
    content = content.substring(0, returnIdx) + selectedUserBlock + '\n' + returnBlock;
  }
  
  // Update InfoCard design slightly for anti-slop
  content = content.replace(
    /const InfoCard = \(\{ icon, label, value \}: any\) => \([\s\S]*?<\/div>\n\);/,
  ...
  // Update StatCard design slightly
  content = content.replace(
    /const StatCard = \(\{ icon, label, value, color \}: any\) => \{[\s\S]*?return \([\s\S]*?<\/div>\n  \);\n\};/,
  ...
  // Update ActionBtn design slightly
  content = content.replace(
    /const ActionBtn = \(\{ icon, label, onClick, color \}: any\) => \{[\s\S]*?return \([\s\S]*?<\/button>\n  \);\n\}/,
  ...
  // Refactor user list styling
  content = content.replace(
    /<div key=\{u\.uid\} onClick=\{\(\) => setSelectedUser\(u\)\} className="flex items-center justify-between p-4 border-b border-slate-100 hover:bg-slate-50 cursor-pointer transition-colors">/g,
    `<div key={u.uid} onClick={() => setSelectedUser(u)} className="flex items-center justify-between p-4 px-6 border-b border-slate-100 hover:bg-indigo-50/50 cursor-pointer transition-colors group">`
  );
