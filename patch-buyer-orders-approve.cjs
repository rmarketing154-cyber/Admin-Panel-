...
const approveRegex = /try \{\s*\/\/\ 1\.\ Call Backend Server API for guaranteed order approval & escrow settlement[\s\S]*?const resData = await res\.json\(\);[\s\S]*?if \(!res\.ok \|\| !resData\.success\) \{[\s\S]*?throw new Error\(resData\.error \|\| 'Server error occurred during approval\.'\);[\s\S]*?\}[\s\S]*?\} catch \(apiErr: any\) \{[\s\S]*?console\.error\('API approve order error:', apiErr\);[\s\S]*?throw apiErr;[\s\S]*?\}/;

content = content.replace(approveRegex, approveLogic.trim());

fs.writeFileSync(file, content);
console.log('Patched approve correctly');
