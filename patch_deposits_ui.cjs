...
const search = `{activeTab === 'my_orders' && (`;
const replace = `{activeTab === 'my_deposits' && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {myDeposits.map((dep: any) => {
                ...
                const parseDate = (val: any) => {
                   ...
                   if (typeof val === 'object' && val['.sv']) return Date.now();
                ...
                return (
                  <div key={dep.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="text-xs text-slate-500 space-y-1">
                      ...
                      <div className="text-[10px] text-slate-400 mt-2">{new Date(parseDate(dep.createdAt)).toLocaleString()}</div>
                    </div>
                  </div>
                );
              })}
...
if (content.includes(search)) {
  ...
  console.log("Patched deposits UI.");
} else {
